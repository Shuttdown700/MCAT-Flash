...
UPLOAD_DIR = os.path.join(BASE_DIR, 'csv_data')
os.makedirs(UPLOAD_DIR, exist_ok=True)


# --- Custom Log Handler ---
class NiceGuiLogHandler(logging.Handler):
    """Pushes log messages to the NiceGUI log element."""
    def __init__(self, gui_log_element):
        super().__init__()
        self.gui_log = gui_log_element

    def emit(self, record):
        try:
            if self.gui_log.client.id in app.clients:
                msg = self.format(record)
                self.gui_log.push(msg)
        except Exception:
            pass
...
def scan_csv_directory_for_conflicts(target_room=None, target_sensor=None):
    ...
    for filename in os.listdir(UPLOAD_DIR):
        ...
        try:
            ...
            app_logger.warning(
                f"Failed to read {filename} during conflict scan: {e}"
            )
...
def precheck_sensor_mac():
    """Quickly grabs the MAC address before the heavy flashing begins."""
    try:
        ports = serial.tools.list_ports.comports()
        detected_port = next(
            (p.device for p in ports if p.vid in [0x303a, 0x1a86]), None
        )
...
async def handle_skip_print_change(e):
    if e.value:
        with ui.dialog() as dialog, ui.card():
            ui.label('Are you sure you want to skip printing labels?').classes(
                'text-lg font-bold'
            )
            ui.label('You will need to manually identify this sensor.').classes(
                'text-gray-600 mb-4'
            )
            with ui.row().classes('w-full justify-end mt-4'):
                ui.button(
                    'Cancel', color='grey', on_click=lambda: dialog.submit(False)
                )
                ui.button(
                    'Yes, Skip', color='negative', on_click=lambda: dialog.submit(True)
                )
...
async def handle_upload(e: events.UploadEventArguments):
    ...
    try:
        ...
        app_logger.error(f"Failed to upload file {e.file.name}: {e}")
...
def delete_selected_file():
    if state.selected_file:
        ...
        try:
            ...
            app_logger.exception(f"Error deleting file {state.selected_file}: {e}")
...
def load_rooms_from_csv():
    ...
    try:
        ...
        if all_full and rooms:
            ui.notify(
                f'All rooms in {state.selected_file} are assigned!', 
                type='warning'
            )
...
def handle_room_change(e):
    ...
    app_logger.info(f"Selected room changed to: {state.selected_room}")
    update_ui_state()

    if e.value:
        is_assigned, assigned_sensor, filename = scan_csv_directory_for_conflicts(
            target_room=e.value
        )
        if is_assigned:
            ui.notify(
                f"Warning: {e.value} is already assigned to {assigned_sensor} "
                f"in {filename}",
                type='warning',
                icon='warning',
                timeout=6000,
...
def update_ui_state():
    if not state.selected_file or not state.selected_room:
        ...
        state.status_message = (
            f"Ready to flash using {state.selected_file} "
            f"and Room: {state.selected_room}"
        )
        state.status_color = "blue"
        state.action_prompt = (
            "PLUG IN MCAT SENSOR WHILE HOLDING THE BOOT BUTTON TO GET STARTED"
        )
        test_button.enable()
...
def is_printer_connected():
    ...
    try:
        ...
        if printer is None:
            all_brother = list(
                usb.core.find(find_all=True, idVendor=0x04f9, backend=backend)
            )
            if all_brother:
                app_logger.info(
                    f"Found {len(all_brother)} Brother device(s), "
                    "but none match PID 0x209b."
                )
            return None
...
async def trigger_flash_process():
    app_logger.info(
        f"Starting flash process || MCAT sensor: {precheck_sensor_mac()}; "
        f"Room: {state.selected_room}; File: {state.selected_file}"
    )

    if not is_sensor_connected():
        app_logger.warning(
            "Flash aborted: MCAT Sensor disconnected immediately before flashing."
        )
        ui.notify('MCAT Sensor disconnected! Please plug it back in.', type='negative')
        return

    # Pre-flight Room Conflict Check
    is_assigned, assigned_sensor, filename = scan_csv_directory_for_conflicts(
        target_room=state.selected_room
    )
    if is_assigned:
        with ui.dialog() as dialog, ui.card().classes('items-center p-6'):
            ui.icon('warning', color='warning').classes('text-5xl mb-2')
            ui.label('Target Room Conflict').classes(
                'text-xl font-bold text-orange-500 mb-2'
            )
            ui.label(
                f'{state.selected_room} is already assigned to {assigned_sensor} '
                f'in {filename}.'
            )
            ui.label('Do you want to override this and flash anyway?').classes(
                'text-gray-600 mb-4'
            )
            with ui.row().classes('w-full justify-end mt-2'):
                ui.button('Cancel', color='grey', on_click=lambda: dialog.submit(False))
                ui.button(
                    'Override', color='negative', on_click=lambda: dialog.submit(True)
                )
    ...
    # Pre-flight Sensor MAC Conflict Check
    ui.notify('Checking sensor hardware ID...', type='info', timeout=2000)
    thing_name = await asyncio.to_thread(precheck_sensor_mac)

    if thing_name:
        is_assigned, assigned_room, filename = scan_csv_directory_for_conflicts(
            target_sensor=thing_name
        )
        if is_assigned:
            with ui.dialog() as dialog, ui.card().classes('items-center p-6'):
                ui.icon('swap_horiz', color='warning').classes('text-5xl mb-2')
                ui.label('Sensor Already Provisioned!').classes(
                    'text-xl font-bold text-orange-500 mb-2'
                )
                ui.label(
                    f'This sensor ({thing_name}) is already assigned to '
                    f'{assigned_room} in {filename}.'
                )
                ui.label(
                    f'Continue flashing {thing_name} to {state.selected_room}?'
                ...
                with ui.row().classes('w-full justify-end mt-2'):
                    ...
                    ui.button(
                        'Continue', color='negative', on_click=lambda: dialog.submit(True)
                    )
    ...
    # Printer check
    printer_info = "SKIPPED" if state.skip_printing else is_printer_connected()
    ...
    try:
        state.status_message = (
            "Flashing in progress..." if state.skip_printing 
            else "Flashing and Printing in progress..."
        )
        ...
        flash_script = os.path.join(os.path.dirname(__file__), 'flash_print.py')
        ...
        app_logger.debug(f"Running flash_print.py with command: {' '.join(cmd)}")
        ...
        state.waiting_for_unplug = True
        app_logger.info(
            f"Successfully flashed MCAT sensor {precheck_sensor_mac()} "
            f"to {state.selected_room}"
        )
        ...
        if not is_sensor_connected():
            app_logger.error(
                f"MCAT Sensor {precheck_sensor_mac()} was likely unplugged "
                "during flashing!"
            )
            state.status_message = "MCAT Sensor Unplugged During Flash!"
            ui.notify(
                'Do not unplug the MCAT Sensor until "Success" is shown.', 
                type='negative'
            )
        else:
...
async def trigger_test_process():
    app_logger.info(
        f"Starting system test process with simulated data for: {state.selected_file}"
    )
    ...
    try:
        ...
        state.status_message = (
            "Test Complete! (Printing Skipped)" if state.skip_printing 
            else "Test Complete! Labels should be printing."
        )
        state.status_color = "positive"
...
async def sensor_watch_loop():
    ...
    if state.waiting_for_unplug:
        if not connected:
            app_logger.debug(
                f"MCAT sensor {precheck_sensor_mac()} unplugged. Resetting state."
            )
...
def setup_ui():
    """Constructs the NiceGUI layout and elements."""
    global file_dropdown, room_dropdown, test_button
    global status_card, status_label, action_label, delete_button

    ui.colors(primary='#2b5cff', secondary='#26da97', accent='#ff5e5e')

    with ui.header().classes(
        'items-center justify-between bg-primary text-white shadow-md'
    ):
        ui.label('MCAT Sensor Flashing & Provisioning Tool').classes(
    ...
    with ui.tab_panels(tabs, value='Flasher').classes('w-full bg-transparent'):
            ...
            with ui.row().classes('w-full p-6 gap-8'):
                ...
                with ui.column().classes('w-1/3 min-w-[300px]'):
                    ...
                    with ui.card().classes('w-full shadow-sm border border-gray-200'):
                        ...
                        ui.label('Select Active CSV').classes(
                        ...
                        ui.label('Select Target Room').classes(
                            'font-semibold text-gray-600'
                        )
                        room_dropdown = ui.select(
                            options=state.rooms,
                            value=state.selected_room,
                            on_change=handle_room_change
                        ).classes('w-full')
                        ...
                        ui.checkbox(
                            'Skip Label Printing',
                            value=state.skip_printing,
                            on_change=handle_skip_print_change
                        ).classes('font-bold text-accent w-full')
                        ...
                        delete_button = ui.button(
                            ...
                            color='negative',
                            icon='delete',
                            on_click=delete_selected_file
                        ).classes('w-full mt-2')

                with ui.column().classes('w-2/3 min-w-[400px] flex-grow items-center'):
                    ui.label('2. Flashing Process').classes(
                        'text-xl font-bold text-gray-700 w-full text-left'
                    )

                    status_card = ui.card().classes(
                        'w-full items-center justify-center p-12 '
                        'transition-colors duration-300 bg-grey-100'
                    )
                    with status_card:
                        status_label = ui.label(state.status_message).classes(
                            'text-3xl font-bold text-center text-gray-800'
                        )
                        action_label = ui.label(state.action_prompt).classes(
                            'text-xl font-bold text-center mt-4 text-gray-600 animate-pulse'
                        )
        ...
        with ui.tab_panel('System Logs').classes('w-full'):
            ui.label('Live Application Logs').classes(
                'text-xl font-bold text-gray-700 mb-4'
            )

            gui_log = ui.log(max_lines=1000).classes(
                'w-full h-[600px] bg-[#1e1e1e] text-[#d4d4d4] font-mono '
                'p-4 rounded-md overflow-y-auto whitespace-pre-wrap'
            )
            ...
            # Clear out previous GUI handlers on reload
            app_logger.handlers = [
                h for h in app_logger.handlers if not isinstance(h, NiceGuiLogHandler)
            ]

            ui_handler = NiceGuiLogHandler(gui_log)
            ...
            ui.context.client.on_disconnect(
                lambda: app_logger.removeHandler(ui_handler)
            )
    ...
    ui.timer(
        0.1, 
        lambda: app_logger.info("=== NICEGUI APPLICATION STARTED ==="), 
        once=True
    )
